...
st.set_page_config(page_title="Your Local RAG Chatbot, powered by LlamaIndex", page_icon="🦙", layout="centered", initial_sidebar_state="auto", menu_items=None)
#openai.api_key = st.secrets.openai_key
#st.title("Your Local RAG Chatbot, powered by ROCm & LlamaIndex 💬🦙")
#st.info("Check out the full tutorial to build this app in our [blog post](https://blog.streamlit.io/build-a-chatbot-with-custom-data-sources-powered-by-llamaindex/)", icon="📃")
st.title("Your Local RAG Chatbot \n powered by ROCm")
st.image("https://www.amd.com/content/dam/amd/en/images/logos/products/amd-rocm-lockup-banner.jpg")
         
# Setting in sidebar
st.sidebar.header("Model")
llm_name=st.sidebar.selectbox("", ("llama3", "qwen:7b", "llama2-chinese", "phi3", "tinyllama"))
...
if "config_init" not in st.session_state:
    ...
    st.session_state["db_collection"] = "UCM_dbc"
...
# Clean the VectroDB
st.sidebar.header("Deleted VectorDB")
...
def load_data():
    with st.spinner(text="Loading and indexing the Streamlit docs – hang tight! This should take 2-3 minutes."):
        ...
        if ((not os.path.exists("./chroma_db/CUM_en_db")) or (st.session_state['rebuild_index'])):
        #if not os.path.exists("./chroma_db/CUM_en_db"):
            index = buid_index(service_context, dbpath="./chroma_db/CUM_en_db")
            st.session_state['rebuild_index'] = False
        else:
            index = load_index(service_context=service_context, dbpath="./chroma_db/CUM_en_db")
